...
export default function ChatTextarea({ onSubmit }: Props) {
  ...
  return (
    <div
      ref={containerRef}
      className="fixed inset-x-0 bottom-0 w-full divide-y border-t spring:divide-pink-900 spring:border-t-pink-900 spring:bg-white winter:divide-blue-900 winter:border-t-blue-900 winter:bg-white spring:dark:divide-pink-100 spring:dark:border-t-pink-100 spring:dark:bg-transparent winter:dark:divide-blue-100 winter:dark:border-t-blue-100 winter:dark:bg-black"
    >
      <div className="flex w-full items-center gap-5 py-2.5 pl-2.5 pr-5">
        <textarea
          ref={textareaRef}
          className="h-6 flex-1 resize-none bg-transparent text-18-R-28 backdrop-filter transition-all scrollbar-hide spring:text-pink-950 spring:placeholder:text-pink-950/50 winter:text-blue-900 winter:placeholder:text-blue-900/50 spring:dark:text-pink-950 spring:dark:placeholder:text-pink-50/80 winter:dark:text-blue-100 winter:dark:placeholder:text-blue-100/30"
          maxLength={140}
          value={message}
          onChange={(e) => setMessage(e.currentTarget.value)}
          onKeyDown={(e) => handleEnter(e.nativeEvent)}
        />
        <div className="flex shrink-0 items-center gap-x-5 spring:[&_svg]:text-pink-950 winter:[&_svg]:text-blue-900 spring:dark:[&_svg]:text-pink-950 winter:dark:[&_svg]:text-blue-100">
          <IconButton
