...
export default function MessageListItem({ data, isSender }: Props) {
    return (
        <li className={`${isSender ? 'ml-auto' : 'mr-auto'} w-max `}>
            ...
            >
                <p
                    className={`${isSender ? 'mr-[50px] rounded-tr-none' : 'ml-[50px] rounded-tl-none'} text-14-R-20 min-w-80 max-w-md whitespace-pre-wrap break-all rounded-3xl bg-primary-50 p-5 text-primary-900`}
                >
                    {data.message}
                </p>
